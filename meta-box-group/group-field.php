...
class RWMB_Group_Field extends RWMB_Field {
	...
	protected static function get_sub_values( $fields, $new, $post_id ) {
		// We need to process all fields because we need to cleanup the template fields data.
		foreach ( $fields as $field ) {
			if ( ! $field['clone'] || $field['type'] === 'group' ) {
				continue;
			}

			$new[ $field['id'] ] = RWMB_Clone::value( $new[$field['id']], [], $post_id, $field );
		}
