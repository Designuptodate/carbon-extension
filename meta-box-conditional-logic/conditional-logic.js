( function( $, rwmb ) {
	...
	class SelectorCache {
		...
		get( selector ) {
			if ( undefined === this.collection[ selector ] ) {
				selector = `${selector}:not(.rwmb-clone-template ${selector})`;
	...
	function getValue( fieldName, selectorCache ) {
		...
		// Try to find the fallback selector
		if ( selectorCache.get( '#' + fieldName + '_1').length ) {
			selector = '#' + fieldName + '_1';
	...
	function getSelector( name, selectorCache ) {
		...
		let selectors = [
			name,
			'#' + name,
			'#' + name + '_1', // First element of the cloneable field
			'[name="' + name + '"]',
			'[name^="' + name + '"]',
			'[name*="' + name + '"]'
		];
